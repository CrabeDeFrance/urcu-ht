...
//! };
//!
//! let mut write = ht.write().mutex().lock().unwrap();
...
pub struct RcuHt<K, V> {
    ...
    mutex: Mutex<RcuHtWriterProtected<K, V>>,
    ...
    V: Clone,
{
    ...
    pub fn new(
        ...
        unsafe {
            ...
            let mutex = Mutex::new(RcuHtWriterProtected::new(urcuht));

            Ok(RcuHt { urcuht, mutex })
        }
    }

    /// Get a read handle. Can be used to read data from hashtable.
    pub fn read(&self) -> RcuHtReader<K, V> {
        RcuHtReader::new(self.urcuht)
    }

    /// Get a write handle. Can be used to read or write data.
    pub fn write(&self) -> RcuHtWriter<K, V> {
        RcuHtWriter::new(&self.mutex)
    }
}
...
/// Per thread object used to provide safe read access to RCU hashtable.
///
...
pub struct RcuHtReader<K, V> {
    phantom_key: PhantomData<K>,
    phantom_val: PhantomData<V>,
    urcuht: *mut urcu_sys::cds_lfht,
}

impl<K, V> RcuHtReader<K, V>
where
    K: Hash + Eq,
    V: Clone,
{
    ...
    pub fn new(urcuht: *mut urcu_sys::cds_lfht) -> Self {
        ...
        // TODO add a reference to Rust RcuHt to be sure it cannot be deleted before main hashtable ?
        RcuHtReader {
...
impl<K, V> Drop for RcuHtReader<K, V> {
    fn drop(&mut self) {
...
/// Per thread object used to provide safe write access to RCU hashtable.
///
/// It registers the current thread if needed (the first reader or writer object triggers the registration).
/// It unregisters the current thread when no more objects are alive in this thread.
pub struct RcuHtWriter<'a, K, V> {
    mutex: &'a Mutex<RcuHtWriterProtected<K, V>>,
}

impl<'a, K, V> RcuHtWriter<'a, K, V>
where
    K: Hash,
{
    fn new(mutex: &Mutex<RcuHtWriterProtected<K, V>>) -> RcuHtWriter<K, V> {
        // manage thread reference counter : if the count is 1 => register this thread
        // Indeed, threads calling add or del API need to be done in registered RCU read-side threads.
        let thread_count = URCU_THREAD_REGISTERED_COUNT.with(|cell| {
            let mut thread_count = cell.get();
            thread_count += 1;
            cell.set(thread_count);
    ...
    pub fn mutex(&self) -> &'a Mutex<RcuHtWriterProtected<K, V>> {
        self.mutex
    }
}

impl<'a, K, V> Drop for RcuHtWriter<'a, K, V> {
    fn drop(&mut self) {
        /* manage thread reference counter : if the count is 0 (last object) => unregister this thread */
        let thread_count = URCU_THREAD_REGISTERED_COUNT.with(|cell| {
            let mut thread_count = cell.get();
...
pub struct RcuHtWriterProtected<K, V> {
    phantom_key: PhantomData<K>,
    phantom_val: PhantomData<V>,
    urcuht: *mut urcu_sys::cds_lfht,
}

impl<'a, K, V> RcuHtWriterProtected<K, V>
where
    K: Hash + Eq + Clone,
    V: Clone,
{
    ...
    fn new(urcuht: *mut urcu_sys::cds_lfht) -> RcuHtWriterProtected<K, V> {
        // return an object containing the pointer to the hashtable
        RcuHtWriterProtected {
            phantom_key: PhantomData,
            phantom_val: PhantomData,
            urcuht,
        }
